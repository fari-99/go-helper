...
func removeExpiredToken(redisSession *redis.Client, username string) (err error) {
    ...
    if len(accessTokenUuids) > 0 {
        for _, accessTokenUuid := range accessTokenUuids {
            _, _ = RemoveRedisSession(redisSession, username, accessTokenUuid)
        }
    }

    if len(refreshTokenUuids) > 0 {
        for _, refreshTokenUuid := range refreshTokenUuids {
            _, _ = RemoveRedisSession(redisSession, username, refreshTokenUuid)
        }
    }
...
func getAllUuid(redisSession *redis.Client, username string) (accessUuids []string, refreshUuids []string, err error) {
    keyRedis := getKeyRedis(username, "")
    ...
    accessUuids, err = redisSession.ZRangeByScore(keyRedis.KeyTotalAccess, getAllScore).Result()
    if err != nil {
        return nil, nil, err
    }

    refreshUuids, err = redisSession.ZRangeByScore(keyRedis.KeyTotalRefresh, getAllScore).Result()
    if err != nil {
        return nil, nil, err
    }
...
func setRedisSession(redisSession *redis.Client, username string, data SessionData) error {
    ...
    if accessExpired, err := getTimeDuration(data.Token.AccessExpiredAt); err != nil {
        err = redisSession.Set(keyRedis.KeyAccess, string(dataMarshal), accessExpired).Err() // automatically expired
        if err != nil {
    ...
    if refreshExpired, err := getTimeDuration(data.Token.RefreshExpiredAt); err != nil {
        err = redisSession.Set(keyRedis.KeyRefresh, string(dataMarshal), refreshExpired).Err() // automatically expired
        if err != nil {
...
func GetAllSessions(redisSession *redis.Client, username string) ([]interface{}, error) {
    _, refreshUuids, err := getAllUuid(redisSession, username)
    if err != nil {
        return nil, err
    }
    ...
    for _, refreshUuid := range refreshUuids {
        user, err := GetCurrentUser(redisSession, refreshUuid)
        if err != nil {
            return nil, err
        }
...
func SetupLoginSession(redisSession *redis.Client, username string, data SessionData) (totalLogin int64, err error) {
    ...
    err = setRedisSession(redisSession, username, data)
    if err != nil {
        return 0, err
    }
...
func DeleteAllSession(redisSession *redis.Client, username string, uuid string) (err error) {
    ...
    accessUuids, refreshUuids, err := getAllUuid(redisSession, username)
    ...
    for _, accessUuid := range accessUuids {
        ...
        _, err = RemoveRedisSession(redisSession, username, accessUuid)
        if err != nil {
            return err
        }
    }
    ...
    for _, refreshUuid := range refreshUuids {
        ...
        _, err = RemoveRedisSession(redisSession, username, refreshUuid)
        if err != nil {
            return err
        }
    }
...
func SetFamily(redisSession *redis.Client, username, oldUuid, newUuid string, expiration int64) (err error) {
    ...
    _, err = redisSession.Set(keyRedis.KeyFamily, string(dataMarshal), timeExpired).Result()
    if err != nil {
        return err
    }
...
func CheckFamily(redisSession *redis.Client, username, oldUuid string) (isUsed bool, err error) {
    ...
    _, err = RemoveRedisSession(redisSession, familyData.Username, familyData.NewUuid)
    if err != nil {
        return true, err
    }
